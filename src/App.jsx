...
const seedParticipants = [
  { id: "p1", name: "Alice L.", role: "Catering", vip:false, allergies: "", presences: { "Vendredi-Dîner": false, "Samedi-Petit-déj": true, "Samedi-Lunch": true, "Samedi-Dîner": true, "Dimanche-Lunch": false }, pilotSessions: [] },
  { id: "p2", name: "Bob P.", role: "Pilote", vip:true, allergies: "Sans gluten", presences: { "Vendredi-Dîner": true, "Samedi-Petit-déj": false, "Samedi-Lunch": true, "Samedi-Dîner": false, "Dimanche-Lunch": true }, pilotSessions: [ { jour: "Samedi", start: "12:00", end: "12:30" } ] },
  { id: "p3", name: "Charly M.", role: "Staff", vip:false, allergies: "", presences: { "Vendredi-Dîner": true, "Samedi-Petit-déj": false, "Samedi-Lunch": true, "Samedi-Dîner": true, "Dimanche-Lunch": true }, pilotSessions: [] },
];
...
const seedArticles = [
  ...
  { id: "a3", type: "Matériel", jour: "", moment: "", article: "BBQ électrique", unite: "", portion: 0, budget: 0, statut: "À préparer", notes: "Rallonges", recipe: [] },
  { id: "a4", type: "Matériel", jour: "", moment: "", article: "Glacière", unite: "", portion: 0, budget: 0, statut: "Rangé", notes: "", recipe: [] },
];

/** ========================
 *  LOCAL STORAGE
 *  ======================== */
const LS_KEY = "catering24_state_v7";
 ...
 *  UI TOKENS
 *  ======================== */
const COLORS = { primary:'#D5222A', primaryDark:'#B01B21', text:'#111827', subtle:'#6b7280', bg:'#ffffff' };
const STATUSES = {
  material: ['À préparer','Prêt','En révision','Déployé','À ranger','Rangé'],
  food: ['À faire','En cours','Servi','Annulé']
};
const MATERIAL_STATUS_BG = {
  'À préparer':'#fde68a', 'Prêt':'#bbf7d0', 'En révision':'#fecaca', 'Déployé':'#bae6fd', 'À ranger':'#fef3c7', 'Rangé':'#e5e7eb'
};
const FOOD_STATUS_BG = { 'À faire':'#fde68a','En cours':'#bae6fd','Servi':'#bbf7d0','Annulé':'#e5e7eb' };
const badgeStyleFor = (status, type) => ({ background: (type==='material'? MATERIAL_STATUS_BG[status]: FOOD_STATUS_BG[status]) || '#e5e7eb' });

const surface = {
  card: { border:'1px solid #e5e7eb', borderRadius:20, background:COLORS.bg, padding:16, marginBottom:16, boxShadow:'0 1px 2px rgba(0,0,0,.04), 0 8px 24px rgba(0,0,0,.06)' },
  dialog: { border:'1px solid #e5e7eb', borderRadius:20, padding:20, maxWidth:760 },
  btnBase: { borderRadius:14, padding:'12px 14px', cursor:'pointer', fontWeight:700, letterSpacing:.2, transition:'all .15s ease' },
  input: { padding:'12px 12px', border:'1px solid #e5e7eb', borderRadius:12, width:'100%', background:'#fff' },
...
const Btn = ({ variant='solid', style, ...props }) => (
  <button {...props} style={{
    ...surface.btnBase,
    border: variant==='solid'? `1px solid ${COLORS.primaryDark}` : variant==='danger'? '1px solid #ef4444' : '1px solid #e5e7eb',
    background: variant==='solid'? COLORS.primary : variant==='danger'? '#fee2e2' : '#fff',
    color: variant==='solid'? '#fff' : variant==='danger'? '#991b1b' : COLORS.text,
    boxShadow: variant==='solid'? '0 6px 16px rgba(213,34,42,.25)': 'none',
    ...style
  }} />
);
...
const Badge = ({ children, tone='neutral', style }) => {
  ...
  return <span style={{ border:'1px solid #e5e7eb', background: (style && style.background) || map[tone] || map.neutral, borderRadius:999, padding:'4px 10px', fontSize:12 }}>{children}</span>
};
  ...
  return (
    <div style={{ display:'grid', gap:14 }}>
      ...
      {role.toLowerCase()==='pilote' && (
        <div>
          <label style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
            ...
            <span>VIP (priorité service)</span>
          </label>
      ...
      <Row gap={10} style={{ justifyContent:'flex-end' }}>
        ...
        <Btn onClick={()=> onSubmit({ ...(initial||{}), name, role, vip, allergies, presences, pilotSessions })}>Enregistrer</Btn>
      </Row>
    </div>
  );
};
...
const ArticleForm = ({ initial, onSubmit, onCancel }) => {
  ...
  const statusOptions = type === 'Matériel' ? STATUSES.material : STATUSES.food;
...
function useIsMobile(){
  ...
  useEffect(()=>{ if (typeof window==='undefined') return; const q=window.matchMedia('(max-width: 768px)'); const h=()=>setM(q.matches); h(); q.addEventListener('change',h); return ()=>q.removeEventListener('change',h); },[]);
  return m;
}
...
export default function App() {
  ...
  const [settings, setSettings] = useState(persisted?.settings || { lunchStart:'12:00', lunchEnd:'13:30' });
  ...
  useEffect(()=>{ if (!supabase) saveState({ participants, articles, settings }); },[participants, articles, settings]);
  ...
  // (Lazy fetch) — on réactivera si tables prêtes côté Supabase
  useEffect(()=>{
    if (!supabase) return;
    (async()=>{
      try {
  ...
  const equipDone = equipment.filter(e=> ['Prêt','Déployé','Rangé','Fait'].includes(e.statut)).length;
  const equipProgress = equipment.length? Math.round(equipDone/equipment.length*100) : 0;
  const equipmentStatusCounts = useMemo(()=> equipment.reduce((acc,e)=>{ acc[e.statut] = (acc[e.statut]||0)+1; return acc; },{}), [equipment]);
  ...
  const lunchStart = toMin(settings.lunchStart), lunchEnd = toMin(settings.lunchEnd);
  const lunchConflicts = participants
    .filter(p=> (p.role||'').toLowerCase()==='pilote')
    .flatMap(p=> (p.pilotSessions||[]).filter(s=> s.jour==='Samedi' && Math.max(toMin(s.start),lunchStart) < Math.min(toMin(s.end),lunchEnd)))
    .length;

  return (
    <div style={{fontFamily:'ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto', padding:isMobile? 16:24, maxWidth:1200, margin:'0 auto'}}>
      <style>{`
        .grid-responsive{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
        @media(max-width:1024px){.grid-responsive{grid-template-columns:repeat(2,minmax(0,1fr));}}
        @media(max-width:768px){.grid-responsive{grid-template-columns:1fr;}}
      `}</style>
      ...
      {/* Tabs */}
      <Row gap={8} wrap style={{ marginBottom:8, position:'relative', zIndex:1 }}>
        {!isMobile && ['dashboard','participants','repas','materiel','planning','config'].map(t=> (
          <Btn key={t} variant={t===tab?'solid':'outline'} onClick={()=>setTab(t)}>{t.toUpperCase()}</Btn>
        ))}
      </Row>
      {/* Bottom Nav (mobile) */}
      {isMobile && (
        <div style={{ position:'fixed', bottom:8, left:8, right:8, background:'#fff', border:'1px solid #e5e7eb', borderRadius:16, boxShadow:'0 10px 24px rgba(0,0,0,.10)', padding:8, display:'flex', gap:6, justifyContent:'space-between' }}>
          {['dashboard','participants','repas','materiel','planning','config'].map(t=> (
            <button key={t} onClick={()=>setTab(t)} style={{ flex:1, padding:'10px 8px', borderRadius:12, border:'1px solid #e5e7eb', background: t===tab? COLORS.primary:'#fff', color: t===tab? '#fff': COLORS.text, fontWeight:700 }}>
              {t.slice(0,1).toUpperCase()+t.slice(1,3)}
            </button>
          ))}
        </div>
      )}
      ...
      {tab==='dashboard' && (
        <div className="grid-responsive">
          <Wrap title="PARTICIPANTS – SAMEDI LUNCH"><div style={{ fontSize:30, fontWeight:900 }}>{nbSamediLunch}</div><div style={surface.subtle}>Basé sur présences</div></Wrap>
          <Wrap title="PIZZA – PORTIONS (SAMEDI LUNCH)"><div style={{ fontSize:30, fontWeight:900 }}>{Math.round(nbSamediLunch * ((articles.find(a=>a.article==='Pizza' && a.jour==='Samedi' && a.moment==='Lunch')?.portion) || 0))}</div><div style={surface.subtle}>Portion(s) total</div></Wrap>
          <Wrap title="BUDGET ESTIMÉ"><div style={{ fontSize:30, fontWeight:900 }}>{budget.toFixed(2)} €</div><div style={surface.subtle}>Repas & Boissons</div></Wrap>
          <Wrap title="MATÉRIEL PRÊT">
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ fontSize:30, fontWeight:900 }}>{equipProgress}%</div>
              <div style={{ flex:1, height:10, background:'#eee', borderRadius:10 }}><div style={{ width:`${equipProgress}%`, height:'100%', background:COLORS.primary }}/></div>
            </div>
            <div style={{ ...surface.subtle, marginTop:6 }}>{equipDone}/{equipment.length} éléments</div>
          </Wrap>
          <Wrap title="TOTAL PARTICIPANTS"><div style={{ fontSize:30, fontWeight:900 }}>{totalParticipants}</div><div style={surface.subtle}>Inscrits</div></Wrap>
          <Wrap title="ARTICLES COURSES"><div style={{ fontSize:30, fontWeight:900 }}>{shoppingCount}</div><div style={surface.subtle}>Lignes à acheter</div></Wrap>
          <Wrap title="CONFLITS (LUNCH)"><div style={{ fontSize:30, fontWeight:900, color: lunchConflicts? COLORS.primary: '#16a34a' }}>{lunchConflicts}</div><div style={surface.subtle}>{lunchConflicts? 'Pilote(s) en roulage': 'RAS'}</div></Wrap>
        </div>
      )}
        ...
        <Wrap title="Participants" right={<Btn variant='outline' onClick={()=>{ setEditingP(null); setOpenP(true); }}>+ Ajouter</Btn>}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2, minmax(0,1fr))', gap:16, marginTop:12 }}>
            {participants.filter(p=> (p.name||'').toLowerCase().includes(q.toLowerCase())).map(p=> (
              <div key={p.id} style={{ ...surface.card, boxShadow:'none', marginBottom:0 }}>
                <Row style={{ justifyContent:'space-between' }}>
                  <div>
                    <div style={{ fontWeight:800 }}>{p.name} {(p.role||'').toLowerCase()==='pilote' && p.vip && (<span title="VIP" style={{ marginLeft:6, color:'#b45309' }}>★</span>)}</div>
                    <div style={{ fontSize:12, ...surface.subtle }}>{p.role}{p.allergies? ` • ${p.allergies}`: ''}</div>
                  </div>
                  <Row gap={6}>
          ...
          {openP && (
            <dialog open style={surface.dialog}>
          ...
          <Wrap title="Articles (Nourriture & Boissons)" right={<Btn variant='outline' onClick={()=>{ setEditingA(null); setOpenA(true); }}>+ Ajouter</Btn>}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2, minmax(0,1fr))', gap:16, marginTop:12 }}>
              {articles.filter(a=> a.type!=="Matériel" && (`${a.type} ${a.article} ${a.jour} ${a.moment}`.toLowerCase().includes(q.toLowerCase()))).map(a=> (
                <div key={a.id} style={{ ...surface.card, boxShadow:'none', marginBottom:0 }}>
                  <Row style={{ justifyContent:'space-between' }}>
                    <div style={{ fontWeight:800 }}>{a.article}</div>
                    <Badge style={badgeStyleFor(a.statut,'food')}>{a.statut}</Badge>
                  </Row>
                  <div style={{ fontSize:12, ...surface.subtle }}>{a.jour||'-'} • {a.moment||'-'} • {a.unite||'-'} × {a.portion||0}</div>
                  ...
                  {!!(a.recipe&&a.recipe.length) && (
                    <div style={{ marginTop:8, fontSize:12, ...surface.subtle }}>Ingrédients par unité : {a.recipe.map(r=>`${r.name} (${r.perUnit} ${r.unit})`).join(', ')}</div>
                  )}
      ...
      {/* Matériel */}
      {tab==='materiel' && (
        <Wrap title="Matériel" right={<Btn variant='outline' onClick={()=>{ setEditingA({ type:'Matériel', statut: 'À préparer'}); setOpenA(true); }}>+ Ajouter</Btn>}>
          {/* Filtres */}
          <Row gap={8} wrap style={{ marginBottom:12 }}>
            {['Tous', ...STATUSES.material].map(s=> (
              <button key={s} onClick={()=>setMatFilter(s)} style={{ padding:'6px 10px', borderRadius:999, border:'1px solid #e5e7eb', background: (matFilter===s? COLORS.primary:'#fff'), color: matFilter===s? '#fff':COLORS.text, fontWeight:700 }}>{s}</button>
            ))}
          </Row>
          {/* Résumé statuts */}
          <Row gap={8} wrap style={{ marginBottom:12 }}>
            {STATUSES.material.map(s=> (
              <Badge key={s} style={badgeStyleFor(s,'material')}>{s}: {equipmentStatusCounts[s]||0}</Badge>
            ))}
          </Row>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2, minmax(0,1fr))', gap:16 }}>
            {equipment.filter(e=> matFilter==='Tous' || e.statut===matFilter).map(e=> (
              <div key={e.id} style={{ ...surface.card, boxShadow:'none', marginBottom:0 }}>
                <Row style={{ justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div>
                    <div style={{ fontWeight:800 }}>{e.article}</div>
                    <div style={{ fontSize:12, ...surface.subtle }}>Statut</div>
                  </div>
                  <Row gap={8}>
                    <Badge style={badgeStyleFor(e.statut,'material')}>{e.statut}</Badge>
                  </Row>
                </Row>
                {/* Changer rapidement le statut */}
                <div style={{ marginTop:8 }}>
                  <Select value={e.statut} onChange={(ev)=> upsertArticle({ ...e, statut: ev.target.value })}>
                    {STATUSES.material.map(s=> <option key={s} value={s}>{s}</option>)}
                  </Select>
                </div>
                <Row gap={8} style={{ marginTop:10, justifyContent:'flex-end' }}>
      ...
      {tab==='planning' && (
        <>
          {/* Tâches cuisine & service */}
          <Wrap title="Tâches cuisine & service (À FAIRE / EN COURS)">
            <div style={{ display:'grid', gap:10 }}>
              {['Vendredi','Samedi','Dimanche'].map(j=> (
                <div key={j}>
                  <div style={{ fontWeight:800, marginBottom:6 }}>{j.toUpperCase()}</div>
                  <div style={{ display:'grid', gap:8 }}>
                    {articles.filter(a=> a.type!=="Matériel" && a.jour===j && ['À faire','En cours'].includes(a.statut)).map(a=> (
                      <Row key={a.id} style={{ justifyContent:'space-between' }}>
                        <div>
                          <div style={{ fontWeight:700 }}>{a.article}</div>
                          <div style={{ fontSize:12, ...surface.subtle }}>{a.moment || '-'} • {a.unite||'-'} × {a.portion||0}</div>
                        </div>
                        <Row gap={6}>
                          <Badge style={badgeStyleFor(a.statut,'food')}>{a.statut}</Badge>
                          <Btn variant='outline' onClick={()=> upsertArticle({ ...a, statut: 'Servi' })}>Marquer SERVI</Btn>
                        </Row>
                      </Row>
                    ))}
                    {articles.filter(a=> a.type!=="Matériel" && a.jour===j && ['À faire','En cours'].includes(a.statut)).length===0 && (
                      <div style={{ fontSize:12, ...surface.subtle }}>Aucune tâche</div>
                    )}
                  </div>
                </div>
              ))}
          ...
          {/* Repas différés pilotes VIP */}
          <Wrap title="Repas différés – Pilotes VIP">
            <div style={{ display:'grid', gap:8 }}>
              {participants.filter(p=> (p.role||'').toLowerCase()==='pilote' && p.vip).flatMap(p=> (p.pilotSessions||[]).filter(s=> Math.max(toMin(s.start), lunchStart) < Math.min(toMin(s.end), lunchEnd)).map(s=> ({p,s}))).map(({p,s}, idx)=> (
                <Row key={idx} style={{ justifyContent:'space-between' }}>
                  <div>
                    <div style={{ fontWeight:700 }}>{p.name} <span title="VIP" style={{ marginLeft:6, color:'#b45309' }}>★</span></div>
                    <div style={{ fontSize:12, ...surface.subtle }}>Fin session: {s.end} • Préparer repas juste après</div>
                  </div>
                  <Badge tone='warning'>À préparer</Badge>
                </Row>
              ))}
              {participants.filter(p=> (p.role||'').toLowerCase()==='pilote' && p.vip).flatMap(p=> (p.pilotSessions||[]).filter(s=> Math.max(toMin(s.start), lunchStart) < Math.min(toMin(s.end), lunchEnd))).length===0 && (
                <div style={{ fontSize:12, ...surface.subtle }}>Aucun repas différé requis</div>
              )}
            </div>
          </Wrap>
              ...
              {participants.filter(p=> (p.pilotSessions||[]).length>0).map(p=> (
                <div key={p.id}>
                  <div style={{ fontWeight:800 }}>
                    {p.name} {(p.role||'').toLowerCase()==='pilote' && p.vip && (<span title='VIP' style={{ marginLeft:6, color:'#b45309' }}>★</span>)}
                    <span style={{ fontSize:12, ...surface.subtle }}> (Pilote)</span>
                  </div>
      ...
      {tab==='config' && (
        <Wrap title="Configuration / Connexion">
          <div style={{ display:'grid', gap:12 }}>
            ...
            <div>
              <div style={{ fontSize:12, ...surface.subtle }}>Supabase URL</div>
              <Input value={url} readOnly />
            </div>
            <div>
              <div style={{ fontSize:12, ...surface.subtle }}>Anon key (masquée)</div>
              <Input value={(key||'').slice(0,10) + '•••' } readOnly />
            </div>
            <div style={{ borderTop:'1px solid #eee', paddingTop:10, marginTop:6 }}>
              <div style={{ fontWeight:800, marginBottom:6 }}>Paramètres repas</div>
              <Row gap={10} wrap>
                <div>
                  <div style={{ fontSize:12, ...surface.subtle }}>Lunch: début</div>
                  <Input value={settings.lunchStart} onChange={e=> setSettings(s=> ({...s, lunchStart: e.target.value}))} />
                </div>
                <div>
                  <div style={{ fontSize:12, ...surface.subtle }}>Lunch: fin</div>
                  <Input value={settings.lunchEnd} onChange={e=> setSettings(s=> ({...s, lunchEnd: e.target.value}))} />
                </div>
              </Row>
            </div>
